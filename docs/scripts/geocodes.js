function tieOnOnClick()
{
  $("#id-table tbody tr").each((index, element) => {
    ...
    $(element).contextmenu((e) => { //right click
      ...
      copyToClipboard("new GeoCode(\""+id+"\", \""+description+"\");");
...
function tableRowAddChildren(tablebody, parentElement, parentID, childrenData, depth)
{
  childrenData.forEach((data) => {

    if(parentID)
      data.id = parentID + "-" + data.id;

    if(data.children != null)
    {
      var elem = $("<tr><td style=\"padding-left:"+(depth*2)+"em\">"+data.name+"</td><td>"+data.id+"</td></tr>");
      tablebody.append(elem);
      tableRowAddChildren(tablebody, elem, data.id, data.children, depth+1);
    }
    else
    {
...
function setTableData(data)
{
  ...
  var top = $("<tr><td>Worldwide</td><td></td></tr>");
...
$(document).ready(()=>{
  $.get({url: "https://trends.google.com/trends/api/explore/pickers/geo?hl=en-GB&tz=0", dataType: "text"})
  .done((text) => {
